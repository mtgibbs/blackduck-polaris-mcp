import { z } from "zod";
import { createIssuePolicy } from "../../services/index.ts";
...
export const createIssuePolicyTool: ToolDefinition<typeof schema> = {
  name: "create_issue_policy",
  description:
    "Create a new issue policy in Polaris. Issue policies trigger actions when security issues match specified filters. Supports max 5 rules per policy, max 5 fixByRule entries per rule. Valid actions: SEND_EMAIL, BREAK_THE_BUILD, CREATE_BUNDLE_JIRA_TICKET. Returns 201 Created with the new policy.",
  schema,
  annotations: { readOnlyHint: false, openWorldHint: true },
  handler: async ({ name, description, filter_groups }) => {
    const policy = await createIssuePolicy({
      name,
      description,
