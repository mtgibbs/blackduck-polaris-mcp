...
export function testScmConnection(params: TestScmConnectionParams): Promise<void> {
  const client = getClient();
  const headers: Record<string, string> = {
    "X-Polaris-SCM-PAT": params.scmPat,
  };
  if (params.scmEmail) headers["X-Polaris-SCM-Email"] = params.scmEmail;
  return client.fetch<void>("/api/integrations/repos/scm/test-connection", {
    method: "GET",
